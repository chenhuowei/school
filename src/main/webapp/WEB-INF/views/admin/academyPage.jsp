...
var store = Ext.create('Ext.data.Store', {
    fields: ["id", "code", "name","head", "officTel","officAddress","status"],
    ...
    proxy: {
        type: 'ajax',
        url: '${pageContext.request.contextPath}/academy/findByPage.do',
...
var grid = Ext.create('Ext.grid.Panel', {
    ...
    columns: [                    
                  ...
                  { text: '学院代码', dataIndex: 'code',  width: 80},
                  { text: '名称', dataIndex: 'name',width:80},
                  { text: '负责人', dataIndex: 'head',width:80,
    ...
    listeners: { 'itemclick': function (view, record, item, index, e) {
     ...
     {text:'excel导入',iconCls:'',handler: function(){openUpload(basePath+'academy/importFromExcel.do','xls/xlsx',store)}},"-",
     {text:'导出excel',iconCls:'',handler: function(){
    	 
    	 window.location.href = basePath+'academy/exportToExcel.do';
     }},"-",
...
function openWin(id,isHidden){
	...
	var icon = '';
	if ('' != id && null != id){
		url = '${pageContext.request.contextPath}/academy/update.do';
		title = '编辑学院';
		icon = 'icon-edit';
	}else{
		url = '${pageContext.request.contextPath}/academy/save.do';
		title = '录入学院';
		icon = 'icon-save';
	...
	var form = new Ext.FormPanel({
	    ...
	    items: [
		{
		    ...
		    items: [
		            ...
		            { xtype:'textfield', name:'code',id:'code', fieldLabel:'学院代码', allowBlank: false,
		            	value : '1'+getNumStart()+'0',disabled:false,regex:/^[1-9]\d{5}$/,
		            	regexText: '请输入以1+年份后两位数字为开头的6位数字(2016 : 116012)'},
	   ...
	   
	    {
	        ...
	        items: [
	                ...
	                { xtype:'textfield', name:'head', fieldLabel:'负责人', allowBlank: false},
	                { xtype:'textfield', name:'headMobile', fieldLabel:'负责人手机', allowBlank: false,vtype:'mobilephone' },
...
function isRepeat(num){
	var b = false;
	$.ajax({
		url : '${pageContext.request.contextPath}/academy/isRepeat.do',
		method: 'get',
		async: false,
		data : {
			code : num
...
function loadData(form,id){
     ...
     form.getForm().load({                    
          waitMsg : '正在加载数据请稍后',//提示信息                
          waitTitle : '提示',//标题                
          url : '${pageContext.request.contextPath}/academy/findById.do',            
	...
	if (rows.length > 0 || (null != oid && '' != oid)) {
		Ext.MessageBox.confirm('确认', '您是否要删除当前选中的项目？', function callback(id) {
			if (id == 'yes') {
				...
				$.ajax({
					url : '${pageContext.request.contextPath}/academy/delete.do',
					method: 'post',
					data : {
						//数组转为 字符串
						ids : ids.join(',')
