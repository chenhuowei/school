...
<jsp:include page="rolePermission.jsp"></jsp:include>
...
var store = Ext.create('Ext.data.Store', {
    fields: ["id", "roleName", "createTime","updateTime", "status","remark"],
    ...
    proxy: {
        type: 'ajax',
        url: '${pageContext.request.contextPath}/roleInfo/findByPage.do',
...
var grid = Ext.create('Ext.grid.Panel', {
    ...
    columns: [                    
                  ...
                  { text: '角色名称', dataIndex: 'roleName',  width: 120},
                  { text: '角色说明', dataIndex: 'remark',width:600},
                  { 
                	  xtype:'actioncolumn',
                      text: '角色权限',
                      width:100,
                  ...
                  { text: '角色状态', dataIndex: 'status',width:80,
...
function openWin(id,isHidden){
	...
	var icon = '';
	if ('' != id && null != id){
		url = '${pageContext.request.contextPath}/roleInfo/update.do';
		title = '编辑角色';
	}else{
		url = '${pageContext.request.contextPath}/roleInfo/save.do';
		title = '录入角色';
	...
	var form = new Ext.FormPanel({
	    ...
	    
	    {
	        ...
	        items: [
	            ...
	            { xtype:'textfield', name:'roleName', fieldLabel:'角色名称', allowBlank: false},
	  ...
	  
	    {
	   	 xtype: "container",
	           layout: "hbox",
	           items: [
	               {xtype:'radiogroup', name:'userAccountStatus', fieldLabel:'角色状态',   allowBlank: false,
	    ...
	    
	    {
	   	 xtype: "container",
	           ...
	           items: [
	               ...
	               { xtype: "textarea", name: "remark", fieldLabel: "角色说明", allowBlank: true ,emptyText : '', grow:true, height:100,width:550,
	                   maxLength : 300,  }
	           ]
	   }, 
	...
	if (rows.length > 0 || (null != oid && '' != oid)) {
		Ext.MessageBox.confirm('确认', '您是否要删除当前选中的项目？', function callback(id) {
			if (id == 'yes') {
				...
				$.ajax({
					url : '${pageContext.request.contextPath}/roleInfo/delete.do',
					method: 'post',
					data : {
						//数组转为 字符串
						ids : ids.join(',')
